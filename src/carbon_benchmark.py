...
class CarbonBenchmark:
    ...
    def benchmark_common_tasks(self, max_tasks=5):
        ...
        self.executor.detect_available_languages()
        ...
        for task_name in selected_tasks:
            # selected_tasks contiene stringhe dei nomi delle task
            ...
            for language in available_languages:
                ...
                print(f"\n📊 Progresso: {current_combination}/{total_combinations} ({progress:.1f}%)")
                
                # Trova il file del codice
                code_file_path = self.executor.find_task_file(task_name, language)
                
                if not code_file_path:
                    print(f"⚠️ File non trovato per {task_name} in {language}")
                    continue
                ...
                try:
                    ...
                    # Pulisci il codice
                    code = self.executor.clean_code_content(code)
                    
                    # Verifica se il codice è eseguibile
                    is_executable, reason = self.is_code_executable(code, language)
                    if not is_executable:
                        print(f"⚠️ Saltato {task_name} in {language}: {reason}")
                        continue
                    ...
                    print(f"❌ Errore lettura file {code_file_path}: {e}")
                    continue
